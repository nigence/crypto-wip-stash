// sha3iuf-try-out.cpp : Defines the entry point for the console application.
//
...
void FIPS202ExampleTest()
{
	...
	sha3_Init384(&c);
	sha3_Update(&c, inputTextBuffer, 100);
	sha3_Update(&c, inputTextBuffer, 50);
	sha3_Update(&c, inputTextBuffer, 25);
	sha3_Update(&c, inputTextBuffer, 12);
	sha3_Update(&c, inputTextBuffer, 7);
	sha3_Update(&c, inputTextBuffer, 3);
	sha3_Update(&c, inputTextBuffer, 2);
	sha3_Update(&c, inputTextBuffer, 1);
...
void SpeedTest( int megabytes )
{
	...
	for (int j = 0; j < 1024; j++)
	{
		...
		sha3_Update(&c, inputTextBuffer, sizeof(inputTextBuffer));
	}
